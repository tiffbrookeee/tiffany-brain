import { NextResponse } from 'next/server';
export const dynamic = 'force-dynamic';
export async function POST(req) {
  try {
    const body = await req.json();
    const messages = body?.messages ?? [];
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey) return NextResponse.json({ error: 'No API key' }, { status: 500 });
    if (!messages.length) return NextResponse.json({ error: 'No messages' }, { status: 400 });
    const res = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': apiKey, 'anthropic-version': '2023-06-01' },
      body: JSON.stringify({
        model: 'claude-haiku-4-5-20251001',
        max_tokens: 1024,
        system: "You are Tiffany's second brain. You know her 5 domains: YPO (mentorship/Justin/EO+YNG mentees/gala), Insurance (life insurance/boss Davin/client tracker), Cafe (Mon+Thu/boss Philip), Vixens N Darlings (her company/pitch comp), TunedInWithTiff (content brand/hooks/science-backed education). Be direct and personal.",
        messages,
      }),
    });
    const data = await res.json();
    if (!res.ok) return NextResponse.json({ error: `API error: ${data?.error?.message ?? JSON.stringify(data)}` }, { status: res.status });
    const reply = data?.content?.[0]?.text;
    if (!reply) return NextResponse.json({ error: 'Empty response', debug: data }, { status: 500 });
    return NextResponse.json({ reply });
  } catch (err) {
    return NextResponse.json({ error: err.message }, { status: 500 });
  }
}
